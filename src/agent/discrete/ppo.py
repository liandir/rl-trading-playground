...
class PPOAgent:
    """
    Proximal Policy Optimization (feedforward, discrete actions).

    Shares a feedforward trunk and splits into a categorical policy head and
    a scalar value head.  Uses the clipped-surrogate objective:

        r_t(theta) = exp(log pi_theta(a_t|s_t) - log pi_theta_old(a_t|s_t))
        L_CLIP = E[ min(r_t * A_t, clip(r_t, 1 - eps, 1 + eps) * A_t) ]
        L      = -L_CLIP + vf_coef * (G_t - V(s_t))^2 - ent_coef * H(pi)

    Advantages are computed by either TD(0) or GAE, once before the update
    loop, then treated as fixed targets across k_epochs of gradient steps.
    """
    ...
    def update(
        ...
        advantages, raw_adv, returns, states, actions, old_log_probs, valid_masks = self.compute_advantages(last_next_state)

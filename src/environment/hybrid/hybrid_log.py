...
class MultiCurrencyEnv:
    """
    Hybrid-action multi-asset trading environment (v02 feature set).
...
class BatchedMultiCurrencyEnv:
    ...
    def _trade(self, actions) -> None:
        ...
        sell_offset = 1 + N
        is_buy  = (actions_d >= 1) & (actions_d < sell_offset)
        is_sell = actions_d >= sell_offset

        # ---- BUY ----
        buy_asset  = (actions_d - 1).clamp(0, N - 1)
        buy_frac   = amount_frac
        ...
        # ---- SELL ----
        sell_asset  = (actions_d - sell_offset).clamp(0, N - 1)
        sell_frac   = amount_frac

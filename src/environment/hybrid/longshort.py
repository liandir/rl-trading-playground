...
class MultiCurrencyEnv:
    ...
    def _get_state(self) -> State:
        ...
        return State(
            time=t_vec, p_rel=self.p_rel.clone(), hl_rel=self.hl_rel.clone(),
            v_rel=self.v_rel.clone(), x_rel=x_rel, c_rel=c_rel, rho=rho,
            unrl_rel=unrl_rel, side_rel=side_rel, vol_rel=self.vol_rel.clone(),
        )
...
class BatchedMultiCurrencyEnv:
    ...
    def _obs(self):
        ...
        x_rel    = torch.cat([x_cash, x_assets], dim=1)
        ...
        return torch.cat([
            t_vec, self.p_rel.view(B, -1), self.hl_rel, x_rel, c_rel, rho[:, None],
            self.v_rel.view(B, -1), unrl_rel, side_rel, self.vol_rel.view(B, -1),
        ], dim=1)

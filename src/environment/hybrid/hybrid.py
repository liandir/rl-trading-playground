...
class MultiCurrencyEnv:
    """
    Hybrid-action multi-asset trading environment.

    Action space:
        discrete action 0            -> hold
        discrete action 1 .. N       -> buy asset i
        discrete action N+1 .. 2*N   -> sell asset i
        continuous action            -> trade fraction in [0, 1]

    Buy semantics:
        spend action_fraction * current cash on selected asset,
        subject to fixed buy fee and minimum buy dollars.

    Sell semantics:
        sell action_fraction * current units of selected asset,
    ...
    def encode_buy(self, asset_idx: int) -> int:
        if not (0 <= asset_idx < self.N):
            raise ValueError("asset_idx out of range")
        return 1 + asset_idx

    def encode_sell(self, asset_idx: int) -> int:
        if not (0 <= asset_idx < self.N):
            raise ValueError("asset_idx out of range")
    ...
    def valid_action_mask(self) -> torch.Tensor:
        ...
        if can_buy:
            for k in range(self.N):
                mask[self.encode_buy(k)] = True

        # The continuous amount is sampled separately; the mask marks actions
        # valid when a full-fraction trade could satisfy the fixed fee.
        for k in range(self.N):
            units = float(self.w[k].item())
            if units > 0.0:
                price = float(self.p[k].item())
                if units * price > self.s_fee:
                    mask[self.encode_sell(k)] = True
    ...
    def _trade(self, a) -> tuple[torch.Tensor, bool]:
        ...
        Returns:
            action_info: tensor [action_type, asset_idx, fraction]
                ...
                fraction:   0 for hold, else clamped continuous action
            valid_trade: bool
        ...
        # --------------------------------------------------
        # BUY: spend continuous fraction of cash on one asset
        # --------------------------------------------------
        if 1 <= action_idx < sell_offset:
            k = action_idx - 1
            ...
            action_info[2] = frac     # action fraction
        ...
        # --------------------------------------------------
        # SELL: sell continuous fraction of held units of one asset
        # --------------------------------------------------
        else:
            k = action_idx - sell_offset
...
class BatchedMultiCurrencyEnv:
    ...
    step((actions_d, actions_c), close, high, low, volume, time)
                                                         -> obs, rewards, dones
    ...
    def _split_actions(self, actions) -> tuple[torch.Tensor, torch.Tensor]:
        if actions is None:
            actions_d = torch.zeros(self.B, dtype=torch.long)
            actions_c = torch.zeros(self.B, dtype=self.dtype)
        elif isinstance(actions, tuple):
            actions_d, actions_c = actions
        elif (
            isinstance(actions, list)
            and len(actions) == 2
            and not all(isinstance(x, (int, float, bool)) for x in actions)
        ):
            actions_d, actions_c = actions
        else:
            actions_d = actions
            actions_c = torch.ones_like(torch.as_tensor(actions_d), dtype=self.dtype)

        actions_d = torch.as_tensor(actions_d).to(device=self.C.device, dtype=torch.long).reshape(-1)
        actions_c = torch.as_tensor(actions_c).to(device=self.C.device, dtype=self.dtype).reshape(-1).clamp(0.0, 1.0)
    ...
    def _trade(self, actions) -> None:
        ...
        actions: tuple of ((B,) discrete int tensor, (B,) amount-fraction tensor).
        Updates C, w, invested, realized_cost, realized_pnl in-place.
        """
        B, N = self.B, self.N
        b = self._b_idx   # (B,)
        actions_d, amount_frac = self._split_actions(actions)

        self.realized_cost = torch.zeros(B, N, dtype=self.dtype)
        self.realized_pnl  = torch.zeros(B, N, dtype=self.dtype)

        sell_offset = 1 + N
        is_buy  = (actions_d >= 1) & (actions_d < sell_offset)     # (B,)
        is_sell = actions_d >= sell_offset                         # (B,)

        # ---- BUY ----
        buy_asset  = (actions_d - 1).clamp(0, N - 1)           # (B,)
        buy_frac   = amount_frac                                # (B,)
        ...
        # ---- SELL ----
        sell_asset  = (actions_d - sell_offset).clamp(0, N - 1) # (B,)
        sell_frac   = amount_frac                                # (B,)
    ...
    def step(
        self,
        actions,                # ((B,) discrete actions, (B,) continuous fractions)
        close: torch.Tensor,    # (B, N)
        high: torch.Tensor,     # (B, N)
        low: torch.Tensor,      # (B, N)
        volume: torch.Tensor,   # (B, N)
    ...
    def valid_action_mask(self) -> torch.Tensor:
        ...
        buy_valid = (self.C > self.b_fee) & ((self.C - self.b_fee) >= self.min_buy_dollars)
        for k in range(N):
            mask[:, self.encode_buy(k)] = buy_valid

        for k in range(N):
            units = self.w[:, k]
            price = self.p[:, k]
            mask[:, self.encode_sell(k)] = (units > 0.0) & (units * price > self.s_fee)

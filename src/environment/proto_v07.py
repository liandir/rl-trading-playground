...
class MultiCurrencyEnv:
    ...
    def _trade(self, a: torch.Tensor | None) -> torch.Tensor:
        ...
        if a is None:
            beta = torch.tensor(0.0, dtype=self.dtype)
            b = torch.full((self.N,), 1.0 / self.N, dtype=self.dtype)
            s = torch.zeros(self.N, dtype=self.dtype)
        else:
            a = a.to(self.dtype)
            beta = torch.sigmoid(a[0])
            b = torch.softmax(a[1:1+self.N] / self.temp, dim=-1)
            s = torch.sigmoid(a[1+self.N:1+2*self.N])

        # 1) SELLS (fractions of units)
        w_before = self.w.clone()
        sell_units = (s * w_before).clamp(min=0.0)
        sell_value = sell_units * self.p

        sells = sell_value > self.s_fee
        if sells.any():
            # recompute with mask
            sell_units_m = torch.minimum(sell_units[sells], w_before[sells])
            sell_value_m = sell_units_m * self.p[sells]

            f = (sell_units_m / w_before[sells]).clamp(0.0, 1.0)
            realized_cost = f * self.invested[sells]

            proceeds = (sell_value_m - self.s_fee).clamp(min=0.0)
            ...
            self.w[sells] -= sell_units_m
            self.invested[sells] -= realized_cost

            self.realized_cost[sells] = realized_cost
            self.realized_pnl[sells]  = realized_pnl

        # 2) BUYS (fraction of cash, split by softmax)
        budget = beta * self.C

        if budget > self.eps:
            buy_dollars = budget * b
            keep = buy_dollars >= self.min_buy_dollars
            buy_dollars = torch.where(keep, buy_dollars, 0.0)
            fee_dollars = torch.where(keep, self.b_fee, 0.0)

            buy_sum = buy_dollars.sum()
            fee_sum = fee_dollars.sum()

            if buy_sum > self.eps and budget > fee_sum + self.eps:
                # scale only notionals to fit budget after fixed fees
                scale = (budget - fee_sum) / (buy_sum + self.eps)
                buy_dollars = buy_dollars * scale

                # units
                buy_units = buy_dollars / self.p.clamp(min=self.eps)
                total_cost = buy_dollars + fee_dollars
                spend = total_cost.sum()

                # safety clamp
                spend = torch.minimum(spend, self.C)
                self.C = self.C - spend
        ...
        # optional: return something interpretable
        return torch.cat([beta.view(1), b, s])
    ...
    def _reward(self):
        ...
        if self.realized_cost.sum() > 0.0:
            ...
            reward = self.dV_coeff * ((self.V - self.V_prev) / self.V_prev).detach().item()
            # reward = self.dV_coeff * torch.log(self.V / self.V_prev).detach().item()
